...
impl Serialize for BlockNumberOrHash {
	...
	{
		match *self {
			BlockNumberOrHash::Hash { hash, require_canonical } => serializer.serialize_str(
...
impl<'a> Visitor<'a> for BlockNumberOrHashVisitor {
		...
		V: MapAccess<'a>,
	{
		...
		if let Some(hash) = block_hash {
			return Ok(BlockNumberOrHash::Hash { hash, require_canonical })
		}
		...
		E: Error,
	{
		match value {
			...
			_ => value.parse::<u64>().map(BlockNumberOrHash::Num).map_err(|_| {
