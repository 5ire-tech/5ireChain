	...
	EC: EthConfig<B, C>,
{
	...
	pub async fn block_info_by_eth_transaction_hash(
		...
		Ok((self.block_info_by_substrate_hash(substrate_hash).await?, index))
	}
	...
	pub async fn block_info_by_substrate_hash(
		...
		let block = self.block_data_cache.current_block(schema, substrate_hash).await;
		let receipts = handler.current_receipts(substrate_hash);
		let statuses =
			self.block_data_cache.current_transaction_statuses(schema, substrate_hash).await;
		let is_eip1559 = handler.is_eip1559(substrate_hash);
		let base_fee = self.client.runtime_api().gas_price(substrate_hash).unwrap_or_default();

		Ok(BlockInfo::new(block, receipts, statuses, substrate_hash, is_eip1559, base_fee))
	}
}
	...
	EC: EthConfig<B, C>,
{
	...
	async fn transaction_by_block_number_and_index(
		...
		self.transaction_by_block_number_and_index(number_or_hash, index).await
	}
...
impl<H> BlockInfo<H> {
	pub fn new(
		...
		Self { block, receipts, statuses, substrate_hash, is_eip1559, base_fee }
	}
}
