	...
	EC: EthConfig<B, C>,
{
	pub async fn transaction_by_hash(&self, hash: H256) -> RpcResult<Option<Transaction>> {
		...
		let BlockInfo { block, statuses, base_fee, .. } =
	...
	pub async fn transaction_by_block_number_and_index(
		...
		let index = index.value();
		let BlockInfo { block, statuses, base_fee, .. } = self.block_info_by_number(number).await?;
	...
	pub async fn transaction_receipt(
		...
		let BlockInfo { block, receipts, statuses, substrate_hash, .. } = block_info.clone();
