...
/// Consensus data provider, pending api uses this trait object for authoring blocks valid for any
/// runtime.
...
mod aura {
	...
	impl<B: BlockT, C: Send + Sync> ConsensusDataProvider<B> for AuraConsensusDataProvider<B, C> {
		fn create_digest(
			...
			let timestamp =
				data.timestamp_inherent_data()?.expect("Timestamp is always present; qed");
