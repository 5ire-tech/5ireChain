...
5irechain is split into multiple levels with increasing opinion and decreasing flexibility:
...
In the lowest level, 5irechain defines primitives, interfaces and traits to implement any on-chain 5irechain transition system and its interactions with the outside world. This is the lowest level of abstraction and opinion that everything else builds upon.
...
In the client you can find a set of crates to construct the outer 5irechain-node, implementing outer runtime interfaces, thus it depends on `runtime`. It provides the outer building blocks like transaction queue, networking layer, database backend, full* and light-client support.
...
* _found in_: `/test`
* _crates prefix_: `5irechain-test`
* _constraints_:
...
The default (testing) application pulling together our recommended setup of 5irechain-client with a wasm-contracts-supporting frame-runtime.  The node pulls it all together, constructs the (upgradable) runtime, and wires up the client around it. You can find an example client, which includes a full wasm-contracts chain in  `node`. This is also what is being built and run if you do `cargo run`.
...
- **subkey**
  Subkey is a client library to generate keys and sign transactions to send to a 5irechain node.
- **chain-spec-builder**
  The chain spec builder builds a chain specification that includes a 5irechain runtime compiled as WASM. To ensure proper functioning of the included runtime compile (or run) the chain spec builder binary in `--release` mode.
