...
describe("Validator tests", function () {
  ...
  it("Should stake to become a validator", async () => {
    ...
    let increaseValidatorCount =
      polkadotApi.tx.staking.increaseValidatorCount(2);
    ...
    await increaseValidatorCountCall.signAndSend(alith, {
      tip: 200,
      nonce: -1,
    });
      ...
      const unsub = await bondValidator.signAndSend(
        ...
        (result) => {
          ...
          if (result.status.isInBlock) {
            console.log(
              `bond validator transaction included at blockHash ${result.status.asInBlock}`,
            );
            console.log(
              `bond validator transaction waiting for finalization... (can take a minute)`,
            );
          } else if (result.status.isFinalized) {
            console.log(
              `bond validator transaction events are ${result.events}`,
            );
            console.log(
              `bond validator transaction finalized at blockHash ${result.status.asFinalized}`,
            );
            unsub();
          }
        },
    ...
    const prefs = polkadotApi.registry.createType(
    ...
    const validateValidatorTransaction = new Promise<{
      block: string;
      address: string;
    }>(async (resolve, reject) => {
      const unsub = await validateValidator.signAndSend(
        baltathar,
        { tip: 200, nonce: -1 },
        (result) => {
          ...
          if (result.status.isInBlock) {
            console.log(
              `validate validator transaction included at blockHash ${result.status.asInBlock}`,
            );
            console.log(
              `validate validator transaction waiting for finalization... (can take a minute)`,
            );
          } else if (result.status.isFinalized) {
            console.log(
              `validate validator transaction events are ${result.events}`,
            );
            console.log(
              `validate validator transaction finalized at blockHash ${result.status.asFinalized}`,
            );
            unsub();
          }
        },
    ...
    const setKeysTransaction = new Promise<{ block: string; address: string }>(
      async (resolve, reject) => {
        const unsub = await setKeys.signAndSend(
          baltathar,
          { tip: 200, nonce: -1 },
          (result) => {
            ...
            if (result.status.isInBlock) {
              console.log(
                `set keys transaction included at blockHash ${result.status.asInBlock}`,
              );
              console.log(
                `set keys transaction waiting for finalization... (can take a minute)`,
              );
            } else if (result.status.isFinalized) {
              console.log(`set keys transaction events are ${result.events}`);
              console.log(
                `set keys transaction finalized at blockHash ${result.status.asFinalized}`,
              );
              unsub();
            }
          },
