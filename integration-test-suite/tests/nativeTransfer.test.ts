...
describe("Native token tests", function () {
  ...
  it("Should transfer native tokens", async () => {
    ...
    // @ts-ignore
    const { nonce: aliceInitialNonce, data: aliceInitialBalance } =
      await polkadotApi.query.system.account(alith.address);
    // Retrieve the account balance & nonce for Bob
    // @ts-ignore
    const { nonce: bobInitialNonce, data: bobInitialBalance } =
      await polkadotApi.query.system.account(baltathar.address);
    // assert that alice initial balance is same as bob initial balance
    expect(
      aliceInitialBalance.free.toBigInt() == bobInitialBalance.free.toBigInt(),
    ).true;
    ...
    const transfer = new Promise<{ block: string; address: string }>(
    ...
    // @ts-ignore
    const { nonce: aliceNonce, data: aliceBalance } =
      await polkadotApi.query.system.account(alith.address);

    // Retrieve the account balance & nonce for Bob
    // @ts-ignore
    const { nonce: bobNonce, data: bobBalance } =
      await polkadotApi.query.system.account(baltathar.address);
    expect(aliceInitialBalance.free.toBigInt() > aliceBalance.free.toBigInt())
      .true;
