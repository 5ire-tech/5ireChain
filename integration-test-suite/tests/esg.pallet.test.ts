...
describe("ESG Pallet Integration tests", function () {
  ...
  it("Should test ESG Pallet", async () => {
    ...
    // wrong json
    const esgDataStringWrong =
      '[{account:"5FLSigC9HGRKVhB9FiEo4Y3koPsNmBmLJbpXg2mp1hXcS59Y"score:"43",},]';

    await insertEsgScoresFailInvalidJson(
      baltathar,
      charleth,
      JSON.stringify(esgDataStringWrong),
    );
    ...
    await registerOracleFailedOracleRegisteredAlreadyForNonSudo(
      alith,
      baltathar,
    );
...
async function registerOracleFailedCallerNotRootOrSudoOracle(
  bob: KeyringPair,
  charlie: KeyringPair,
) {
  ...
  const unsub = await transaction.signAndSend(
    ...
    (result) => {
      ...
      if (result.status.isInBlock) {
        ...
        const filteredErrorData = dataStr.filter(
          (item: any) => item.event.index === "0x0001",
        );
        expect(filteredErrorData[0].event.data[0].module.error).to.equal(
          "0x08000000",
        );
        console.log(
          `Error found: ${filteredErrorData[0].event.data[0].module.error}`,
        );
      }
    },
  );
...
async function registerOracleFailedOracleRegisteredAlready(
  alice: KeyringPair,
  bob: KeyringPair,
) {
  ...
  const unsub = await api.tx.sudo
    ...
    .signAndSend(alice, { tip: 200, nonce: -1 }, (result) => {
      ...
      if (result.status.isInBlock) {
        ...
        const dataStr = JSON.parse(data);
        const filteredErrorData = dataStr.filter(
          (item: any) => item.event.index === "0x1300",
        );
        expect(filteredErrorData[0].event.data[0].err.module.error).to.equal(
          "0x07000000",
        );
        console.log(
          `Error found: ${filteredErrorData[0].event.data[0].err.module.error}`,
        );
      }
    });
  await sleep(12000);
...
async function deRegisterOracle(alice: KeyringPair, bob: KeyringPair) {
  ...
  const transaction = api.tx.esgScore.deregisterAnOracle(bob.address, true);
...
async function deRegisterOracleForOracleNotExist(
  alice: KeyringPair,
  charlie: KeyringPair,
) {
  console.log(`\nDe-Registering Oracle fail due to OracleNotExist`);

  const transaction = api.tx.esgScore.deregisterAnOracle(charlie.address, true);

  const unsub = await api.tx.sudo
    .sudo(transaction.method.toHex())
    .signAndSend(alice, { tip: 200, nonce: -1 }, (result) => {
      console.log(
        `Oracle De-Registration for OracleNotExist is ${result.status}`,
      );
      if (result.status.isInBlock) {
        console.log(
          `Oracle Registration at blockHash ${result.status.asInBlock}`,
...
async function deRegisterOracleFromBadOrigin(
  bob: KeyringPair,
  charlie: KeyringPair,
) {
  ...
  const unsub = await transaction.signAndSend(
    ...
    (result) => {
      console.log(
        `Oracle De-Registration for FromBadOrigin is ${result.status}`,
      );
      if (result.status.isInBlock) {
        console.log(
          `Oracle Registration at blockHash ${result.status.asInBlock}`,
        ...
        console.log(
          `Error found: ${filteredErrorData[0].event.data[0].badOrigin}`,
        );
      }
    },
  );
...
// Register the Bob account as oracle in ESG pallet failed for already registered account.
async function registerOracleFailedOracleRegisteredAlreadyForNonSudo(
  alice: KeyringPair,
  bob: KeyringPair,
) {
  console.log(
