...
async function payoutAlreadyClaimed(alice: KeyringPair, aliceStash: KeyringPair) {
      ...
      (result) => {
        ...
        if (result.status.isInBlock) {
          ...
          if (result.dispatchError){
            ...
            expect(filteredData[0].event.data[0].module.token).to.equal("AlreadyClaimed");// AlreadyClaimed
          }
