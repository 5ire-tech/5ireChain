...
fn batch_returns(
	precompiles: &MockPrecompileSet<Runtime>,
	mode: Mode,
) -> PrecompilesTester<MockPrecompileSet<Runtime>> {
	let mut counter = 0;
	...
	precompiles
		...
		.with_subcall_handle(move |subcall| {
			let Subcall { address, transfer, input, target_gas, is_static, context } = subcall;
...
fn batch_out_of_gas(
	...
	precompiles
		...
		.with_subcall_handle(move |subcall| {
			...
			match address {
				a if a == Bob.into() => {
					assert_eq!(target_gas, Some(50_000 - total_call_cost), "batch forward all gas");
					let transfer = transfer.expect("there is a transfer");
					assert_eq!(transfer.source, Alice.into());
					assert_eq!(transfer.target, Bob.into());
					assert_eq!(transfer.value, 1u8.into());
					...
					SubcallOutput { cost: 11_000, ..SubcallOutput::out_of_gas() }
				},
				_ => panic!("unexpected subcall"),
			}
		})
...
fn batch_incomplete(
	...
	precompiles
		.prepare_test(
			...
			PCall::batch_from_mode(
				mode,
				vec![Address(Bob.into()), Address(Charlie.into()), Address(Alice.into())],
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_ok!(RuntimeCall::Evm(evm_call(
				Alice,
				PCall::batch_some {
					to: vec![Address(Bob.into()), Address(Charlie.into()), Address(David.into()),]
						.into(),
					value: vec![U256::from(9_000u16), U256::from(2_000u16), U256::from(500u16)]
						.into(),
					call_data: vec![].into(),
					gas_limit: vec![].into()
				}
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_ok!(RuntimeCall::Evm(evm_call(
				Alice,
				PCall::batch_some {
					to: vec![Address(Bob.into()), Address(Revert.into()), Address(David.into()),]
						.into(),
					value: vec![U256::from(1_000u16), U256::from(2_000), U256::from(3_000u16)]
						.into(),
					call_data: vec![].into(),
					gas_limit: vec![].into()
				}
