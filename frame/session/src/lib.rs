...
pub trait AllSessionHandler<ValidatorId> {
	...
	/// `changed` is true whenever any of the session keys or underlying economic
	/// identities or weightings behind those keys has changed.
	fn on_new_session_all<Ks: OpaqueKeys>(
		changed: bool,
		validators: &[(ValidatorId, Ks)],
		queued_validators: &[(ValidatorId, Ks)],
	);
...
#[impl_trait_for_tuples::impl_for_tuples(1, 30)]
#[tuple_types_custom_trait_bound(OneSessionHandlerAll<AId>)]
impl<AId> AllSessionHandler<AId> for Tuple {
	for_tuples!(
		const KEY_TYPE_IDS: &'static [KeyTypeId] = &[ #( <Tuple::Key as RuntimeAppPublic>::ID ),* ];
	);
	...
	fn on_new_session_all<Ks: OpaqueKeys>(
		...
		for_tuples!(
			#(
				...
				Tuple::on_new_session_all(changed, our_keys, queued_keys);
			)*
		)
	}
}
...
pub mod pallet {
	...
	#[pallet::storage]
	#[pallet::getter(fn all_next_keys)]
...
impl<T: Config> Pallet<T> {
	...
	pub fn rotate_session() {
		...
		let all_changed = <AllQueuedChanged<T>>::get();
		...
		// Get next validator set.
		// 5ire's implementation
		let maybe_next_validators_all = T::SessionManager::new_session(session_index + 1);
		let (next_validators_all, next_identities_changed_all) =
			if let Some(all_validators) = maybe_next_validators_all {
				// NOTE: as per the documentation on `OnSessionEnding`, we consider
				// the validator set as having changed even if the validators are the
				// same as before, as underlying economic conditions may have changed.
				(all_validators, true)
		...
		let (queued_amalgamated1, next_changed1) = {
			// until we are certain there has been a change, iterate the prior
			// validators along with the current and check for changes
			let mut all_changed = next_identities_changed_all;

			let mut now_session_keys = all_session_keys.iter();
			let mut check_next_changed = |keys: &T::Keys| {
				if all_changed {
					return
				}
			...
			let queued_amalgamated1 = next_validators_all
				.into_iter()
				.filter_map(|a| {
					let k = Self::load_keys(&a)?;
					check_next_changed(&k);
