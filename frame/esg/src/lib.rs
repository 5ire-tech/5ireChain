...
pub mod pallet {
	...
	use bs58;
	use frame_support::{inherent::Vec, pallet_prelude::{*, DispatchResult}, traits::ERScoresTrait, WeakBoundedVec};
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn register_an_oracle(
			...
			frame_support::log::info!("#@! root: {is_root} same?: {}", acc_id.eq(&oracle));
