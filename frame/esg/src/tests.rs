...
fn it_must_register_oracles_new() {
	...
	assert_ok!(
		Esg::register_an_oracle(RuntimeOrigin::signed(SUDO_ORACLE), NON_SUDO_ORACLE_6, false)
	);
	assert_ok!(
		Esg::register_an_oracle(RuntimeOrigin::signed(SUDO_ORACLE), NON_SUDO_ORACLE_8, false)
	);
	assert_ok!(
		Esg::register_an_oracle(RuntimeOrigin::signed(SUDO_ORACLE), NON_SUDO_ORACLE_9, false)
	);
	assert_ok!(
		Esg::register_an_oracle(RuntimeOrigin::signed(SUDO_ORACLE), NON_SUDO_ORACLE_10, false)
	);
	assert_ok!(
		Esg::register_an_oracle(RuntimeOrigin::signed(SUDO_ORACLE), NON_SUDO_ORACLE_7, false)
	);

	assert_ok!(
		Esg::deregister_an_oracle(RuntimeOrigin::root(), NON_SUDO_ORACLE_7, false)
	);
...
fn it_must_register_oracles() {
	new_test_ext().execute_with(|| {
        ...
        System::assert_last_event(
			RuntimeEvent::Esg(crate::Event::NewOracleRegistered { 
				is_sudo: true,
				oracle: SUDO_ORACLE, 
			})
		);
		...
		assert_ok!(
			Esg::register_an_oracle(RuntimeOrigin::signed(SUDO_ORACLE), SUDO_ORACLE_2, true)
		);
		...
		assert_ok!(
			Esg::register_an_oracle(RuntimeOrigin::signed(SUDO_ORACLE), NON_SUDO_ORACLE_2, false)
		);
...
fn it_must_deregister_oracles_with_root_only() {
	new_test_ext().execute_with(|| {
		...
		// check `NewOracleRegistered` was trigered with expected data
        System::assert_last_event(
			RuntimeEvent::Esg(crate::Event::OracleDeRegistered { 
				is_sudo: true,
				oracle: SUDO_ORACLE, 
			})
		);
		...
		assert_ok!(
			Esg::deregister_an_oracle(RuntimeOrigin::root(), NON_SUDO_ORACLE, false)
		);
...
fn it_must_upload_all_valid_esg_data_with_no_skipping() {
	new_test_ext().execute_with(|| {
        ...
        let company1= AccountId32::from_str("5FHneW46xGXgs5mUiveU4sbTyGBzmstUspZC92UhjJM694ty").unwrap();
        let company2= AccountId32::from_str("5DAAnrj7VHTznn2AWBemMuyBwZWs6FNFjdyVXUeYum3PTXFy").unwrap();
		...
		System::assert_last_event(
			RuntimeEvent::Esg(crate::Event::ESGStored { 
				caller: ROOT 
			})
		);
...
fn it_must_skip_for_invalid_addresses_in_esg_data() {
	new_test_ext().execute_with(|| {
		...
		let valid_id1 = AccountId32::from_str("5FHneW46xGXgs5mUiveU4sbTyGBzmstUspZC92UhjJM694ty").unwrap();
		let valid_id2 = AccountId32::from_str("5DAAnrj7VHTznn2AWBemMuyBwZWs6FNFjdyVXUeYum3PTXFy").unwrap();
		...
		System::assert_last_event(
			RuntimeEvent::Esg(crate::Event::ESGStoredWithSkip { 
				caller: SUDO_ORACLE, 
...
fn it_must_handle_invalid_esg_scores() {
	new_test_ext().execute_with(|| {
		...
		let company1 = AccountId32::from_str("5FHneW46xGXgs5mUiveU4sbTyGBzmstUspZC92UhjJM694ty").unwrap();
		let company2 = AccountId32::from_str("5DAAnrj7VHTznn2AWBemMuyBwZWs6FNFjdyVXUeYum3PTXFy").unwrap();
		let company3 = AccountId32::from_str("5FhTgr75zm6QC4ohRHKFcaaMVnjifTaRcaYeGKBPBBbSSBtK").unwrap();
